...
reg <- lm(ano ~ tempo_emprego, data = tempo)
summary(reg)
tempo1 <- tempo[-(1:16),]
...
summary(reg)
idade1 <- idade[-(18:28),]
reg_queda <- lm(ano ~ idade, data = idade1)
summary(reg_queda)
idade2 <- idade[-(1:16),]
reg_aumento <- lm(ano ~ idade, data = idade2)
summary(reg_aumento)
...
#Salário
reg <- lm(ano ~ remuneracao, data = salário)
summary(reg)
salario1 <- salário[-(1:13),]
reg_queda <- lm(ano ~ remuneracao, data = salario1)
summary(reg_queda)
